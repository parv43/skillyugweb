  ...
  return (
    <div className="mx-auto w-full max-w-3xl [perspective:1400px]">
      <div
        ...
        className="group relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white/95 shadow-lg shadow-purple-500/[0.03] backdrop-blur-xl transition-transform duration-200 ease-out will-change-transform [transform:perspective(1400px)_rotateX(var(--rx))_rotateY(var(--ry))] [transform-style:preserve-3d]"
      >
        ...
        <div className="relative p-4 sm:p-6">
          <div className="relative h-72 overflow-hidden rounded-[1.5rem] border border-slate-200 bg-[radial-gradient(circle_at_50%_40%,rgba(99,102,241,0.08),transparent_30%),linear-gradient(135deg,#f8fafc_0%,#eef2ff_55%,#e0e7ff_100%)] shadow-sm [transform:translateZ(42px)]">
            ...
            <div className="absolute left-1/2 top-[47%] h-44 w-[30rem] -translate-x-1/2 -translate-y-1/2 rounded-full bg-slate-200/40 blur-2xl" />
            ...
            <div className="absolute right-5 top-5 flex items-center gap-2 rounded-full border border-purple-200 bg-purple-50 px-3 py-2 text-xs font-semibold text-purple-700 backdrop-blur-md">
              <Sparkles className="h-4 w-4 text-purple-600" />
              Early Bird
            </div>
          </div>

          <div className="relative mx-auto -mt-16 max-w-xl rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-md backdrop-blur-xl sm:p-8 [transform:translateZ(86px)]">
            <div className="text-center">
              <p className="text-xs font-bold uppercase tracking-[0.26em] text-purple-600">
                Skillyug AI Bootcamp
              </p>
              <div className="mt-4 flex items-end justify-center gap-4">
                <span className="text-xl text-slate-400 line-through">₹4000</span>
                <span className="text-5xl font-bold text-slate-900">{BOOK_SLOT_AMOUNT_LABEL}</span>
              </div>
            </div>

            <div className="mt-8 flex flex-col gap-4">
              {benefits.map((benefit) => (
                <div key={benefit} className="flex items-start gap-3 text-slate-600">
...
function DesktopBootcampHero() {
  return (
    <section className="relative hidden min-h-screen items-center overflow-hidden px-6 pb-16 pt-28 md:flex md:px-10 md:py-28 lg:px-16">
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_20%_26%,rgba(168,85,247,0.04),transparent_34%),radial-gradient(circle_at_78%_30%,rgba(59,130,246,0.03),transparent_34%),radial-gradient(circle_at_78%_70%,rgba(124,58,237,0.04),transparent_38%),linear-gradient(135deg,#f8fafc_0%,#f8fafc_42%,#f3e8ff_72%,#e0f2fe_100%)]" />
      <div className="absolute inset-0 z-0 [contain:layout_paint] md:opacity-80">
        <BootcampSpline />
        <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(90deg,#f8fafc_0%,#f8fafc_34%,rgba(248,250,252,0.84)_48%,rgba(248,250,252,0.34)_68%,rgba(248,250,252,0.18)_100%)]" />
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_74%_46%,transparent_0%,transparent_28%,rgba(248,250,252,0.2)_54%,#f8fafc_100%)]" />
        <div className="pointer-events-none absolute bottom-5 right-5 h-14 w-44 rounded-2xl bg-transparent" />
      </div>
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-48 bg-gradient-to-b from-transparent via-slate-50/80 to-slate-50" />

      <div className="pointer-events-none relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-10">
        <div className="pointer-events-auto max-w-xl">
          <div
            className="mb-6 inline-flex items-center rounded-full border border-purple-200 bg-purple-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-purple-700 backdrop-blur-md"
          >
            Skillyug AI Bootcamp
          </div>
          <h1
...
function MobileBootcampHero() {
  return (
    <section className="relative flex min-h-screen flex-col justify-center overflow-hidden px-6 pb-16 pt-32 md:hidden">
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_50%_0%,rgba(168,85,247,0.06),transparent_40%),radial-gradient(circle_at_80%_80%,rgba(37,99,235,0.04),transparent_40%),linear-gradient(135deg,#f8fafc_0%,#f5f3ff_50%,#f8fafc_100%)]" />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-48 bg-gradient-to-b from-transparent via-slate-50/80 to-slate-50" />

      <div className="relative z-10 mx-auto flex w-full max-w-xl flex-col items-center text-center">
        <div
          className="mb-6 inline-flex items-center justify-center rounded-full border border-purple-200 bg-purple-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-purple-700 backdrop-blur-md"
        >
          Skillyug AI Bootcamp
        </div>
        <h1
...
export default function BootcampPage() {
  return (
    <main className="min-h-screen overflow-hidden bg-slate-50 text-slate-800 selection:bg-purple-100 selection:text-purple-900">
      <Navbar />
            ...
            {instructors.map((instructor, idx) => (
              ...
              >
                ...
                {instructor.image ? (
                  ...
                  <div className="mb-7 flex h-24 w-24 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-2xl font-bold text-slate-700 shadow-sm">
                    {instructor.initials}
                  </div>
                )}
                <div className="pr-8">
                  <h3 className="text-2xl font-bold text-slate-900">{instructor.name}</h3>
                  <p className="mt-2 text-sm font-semibold uppercase tracking-[0.16em] text-purple-600">
                    {instructor.title}
                  </p>
                  {Array.isArray(instructor.bio) ? (
                    <ul className="mt-5 space-y-2 text-base leading-7 text-slate-600 list-disc list-outside ml-4">
                      {instructor.bio.map((point, pointIdx) => {
                        const colonIndex = point.indexOf(':');
                        if (colonIndex !== -1) {
                          return (
                            <li key={pointIdx}>
                              <span className="font-bold text-slate-900">{point.substring(0, colonIndex + 1)}</span>
                    ...
                    <p className="mt-5 text-base leading-7 text-slate-600">{instructor.bio}</p>
                  )}
                </div>
              </article>
            ))}
      ...
      <section className="relative px-6 py-20 md:px-10 lg:px-16">
        <div className="absolute inset-x-0 top-1/2 h-[34rem] -translate-y-1/2 bg-[radial-gradient(circle,rgba(168,85,247,0.05),transparent_62%)]" />
        <div className="relative mx-auto max-w-6xl">
          <h2
            className="mb-10 text-center text-3xl font-bold text-slate-900 sm:text-4xl"
          >
      ...
      <footer className="relative border-t border-slate-200/80 bg-slate-100/50 px-6 py-24 text-center sm:py-32">
        ...
        <div className="relative mx-auto flex max-w-4xl flex-col items-center">
          ...
          <nav
            className="mt-20 flex flex-wrap items-center justify-center gap-x-10 gap-y-5 text-xl font-bold text-slate-600 sm:text-2xl"
          >
          ...
          >
            <Link href="/refund-policy" className="transition hover:text-slate-800">
              Refund Policy
            </Link>
            <span>|</span>
            <Link href="/terms-and-conditions" className="transition hover:text-slate-800">
              Terms & Conditions
            </Link>
          </div>
          <p
            className="mt-16 text-lg font-medium uppercase tracking-[0.28em] text-slate-400 sm:text-2xl"
          >
            © 2026 Skillyug
            <br />
            All Rights Reserved.
