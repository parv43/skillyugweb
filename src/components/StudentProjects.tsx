...
const PROJECT_COLORS: Record<ProjectColor, { badge: string; border: string; text: string }> = {
  blue: { badge: "bg-blue-500/10", border: "border-blue-500/30", text: "text-blue-400" },
  yellow: { badge: "bg-yellow-500/10", border: "border-yellow-500/30", text: "text-yellow-400" },
  pink: { badge: "bg-pink-500/10", border: "border-pink-500/30", text: "text-pink-400" },
  purple: { badge: "bg-purple-500/10", border: "border-purple-500/30", text: "text-purple-400" },
  emerald: { badge: "bg-emerald-500/10", border: "border-emerald-500/30", text: "text-emerald-400" },
  orange: { badge: "bg-orange-500/10", border: "border-orange-500/30", text: "text-orange-400" },
}
...
const ProjectCard = ({ title, tech, desc, icon: Icon }: ProjectCardProps) => {
  return (
    <div className="glass-panel p-6 sm:p-8 rounded-[2rem] border border-white/5 hover:border-blue-500/30 transition-all duration-300 group flex flex-col h-full cyber-glow relative overflow-hidden bg-white/[0.02]">
    ...
    <section id="projects" className="relative w-full py-16 md:py-32 bg-[#020617] overflow-hidden flex flex-col border-t border-white/5">
      ...
      <div className="flex flex-col gap-3 px-4 max-w-lg mx-auto w-full z-10 relative md:hidden">
        {projects.map((project, idx) => {
          ...
          return (
            <div key={idx} className="flex items-start gap-4 bg-[#0f172a]/70 border border-white/8 rounded-2xl p-4">
