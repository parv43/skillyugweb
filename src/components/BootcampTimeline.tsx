...
export default function BootcampTimeline() {
  ...
  useEffect(() => {
    ...
    function showAll() {
      ...
      if (hLineRef.current) {
        hLineRef.current.style.transition = "none"
        hLineRef.current.style.transform = "scaleX(0)"
        // Force reflow so transition reset takes effect
        void hLineRef.current.offsetWidth
        hLineRef.current.style.transition = "transform 1.8s cubic-bezier(0.4, 0, 0.2, 1) 0.2s"
        hLineRef.current.style.transform = "scaleX(1)"
      }
      // Animate vertical line
      if (vLineRef.current) {
        vLineRef.current.style.transition = "none"
        vLineRef.current.style.transform = "scaleY(0)"
        void vLineRef.current.offsetWidth
        vLineRef.current.style.transition = "transform 1.8s cubic-bezier(0.4, 0, 0.2, 1) 0.2s"
        vLineRef.current.style.transform = "scaleY(1)"
      }
    }
    ...
    function hideAll() {
      ...
      // Reset horizontal line instantly
      if (hLineRef.current) {
        hLineRef.current.style.transition = "none"
        hLineRef.current.style.transform = "scaleX(0)"
      }
      // Reset vertical line instantly
      if (vLineRef.current) {
        vLineRef.current.style.transition = "none"
        vLineRef.current.style.transform = "scaleY(0)"
      }
    ...
    >
      ...
      <div className="w-full max-w-7xl mx-auto px-4 relative flex flex-col md:flex-row justify-between items-start gap-8 md:gap-0">
        ...
        <div
          ...
          style={{ top: "32px", left: "10%", right: "10%", height: "2px", zIndex: 0, overflow: "hidden" }}
        >
          <div
            ref={hLineRef}
            style={{
              width: "100%",
              height: "100%",
              background: "linear-gradient(to right, transparent, #3b82f6 40%, #3b82f6 60%, transparent)",
        ...
        <div
          ...
          style={{ top: "32px", bottom: "32px", left: "50%", marginLeft: "-1px", width: "2px", zIndex: 0, overflow: "hidden" }}
        >
          <div
            ref={vLineRef}
            style={{
              width: "100%",
              height: "100%",
              background: "linear-gradient(to bottom, transparent, #3b82f6 40%, #3b82f6 60%, transparent)",
