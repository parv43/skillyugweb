...
export default function DemoBookingSection() {
  ...
  // RAF counter from 0 to 84 — replaces framer-motion's animate()
  useEffect(() => {
    if (!isInView) return
    ...
    const tick = (now: number) => {
      ...
      setPercent(Math.round(eased * 84))
          ...
          <div className="w-full lg:w-[480px] shrink-0">
            ...
            >
              ...
              <div className="absolute top-4 -right-12 w-48 h-10 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 transform rotate-45 flex items-center justify-center shadow-lg pointer-events-none z-20">
                <span className="text-white font-black text-[10px] tracking-[0.2em] uppercase drop-shadow-md">LIVE DEMO</span>
              </div>
              ...
              {/* Progress Bar — CSS transition replaces framer-motion animate */}
              <div className="mb-10 bg-[#020617]/50 rounded-2xl p-5 border border-white/5">
                ...
                <div className="w-full h-3 bg-slate-800/80 rounded-full overflow-hidden relative border border-white/5">
                  <div 
                    className="absolute top-0 bottom-0 left-0 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 rounded-full origin-left will-change-transform shadow-[0_0_12px_rgba(139,92,246,0.3)] transition-transform duration-[1500ms] ease-out overflow-hidden"
                    style={{ transform: isInView ? "scaleX(0.84)" : "scaleX(0)" }}
                  >
