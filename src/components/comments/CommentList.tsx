  ...
  if (allComments.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 py-12 text-center">
        ...
        <p className="text-slate-600 dark:text-slate-400 font-medium text-sm">No comments yet</p>
        <p className="text-slate-500 dark:text-slate-500 text-xs">Be the first to share your thoughts on this article!</p>
      </div>
    );
  }
      ...
      {hasMore && (
        <div className="pt-4 flex justify-center">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2 rounded-full text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 bg-slate-100 dark:bg-white/[0.03] hover:bg-slate-200 dark:hover:bg-white/[0.06] transition-all disabled:opacity-40"
          >
            {isLoadingMore ? (
              <>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2 mb-1.5 flex-wrap">
          <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">{comment.user_name}</span>
          <time
            dateTime={comment.created_at}
            className="text-[11px] text-slate-500 dark:text-slate-500 font-mono flex-shrink-0"
          >
        ...
        <p className="text-sm text-slate-700 dark:text-slate-400 leading-relaxed break-words">{comment.content}</p>
      </div>
    </article>
  );
}
